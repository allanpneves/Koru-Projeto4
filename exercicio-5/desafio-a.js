...
const displayFormatedList = (inventory) => {
    console.log("\nLista de Produtos:");
    inventory.forEach((item) => {
        console.log(`ID: ${item.id}, Nome: ${item.name}, Preço: R$${item.price.toFixed(2)}, Estoque: ${item.stock}, Categoria: ${item.category}`);
    });
}

displayFormatedList(inventory);
